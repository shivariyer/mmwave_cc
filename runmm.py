...
class Simulation(object):
    ...
    def __init__(self, trace, port=None, ttr=None, n_blks=None, filepath=None, blksize=None, cc_algo=None, buf_len=None, iperf=False):
        '''
        ...
        if (ttr is not None) + (n_blks is not None) + (filepath is not None) > 1:
            raise Exception(
                'only one of \'ttr\', \'n_blks\' and \'filepath\' may be specified')
    ...
    def run(self, savedir='output', mm_side='receiver', log=False, verbose=0, disp_plot=False, save_plot=True):
        ...
        # (i) receiver command
        receiver_cmd = 'sender_receiver/receiver {} 1 {}'.format(
            self.port, int(verbose >= 1))
        if log:
            recvlogdir = savepathprefix + '_receiver'
            if not os.path.exists(recvlogdir):
                os.mkdir(recvlogdir)
            receiver_cmd += ' --log {}'.format(
                os.path.join(recvlogdir, 'recvlog'))
        ...
        if mm_side == 'sender':
            # MAHIMAHI_BASE: address exposed by native machine to processes inside mahimahi
            ...
            if verbose >= 2:
                receiver_process = sp.Popen(
                    receiver_cmd.split(), universal_newlines=True)
            else:
                receiver_process = sp.Popen(receiver_cmd.split(
                ), stdin=sp.PIPE, stdout=sp.PIPE, universal_newlines=True)
            ...
            if verbose >= 2:
                sender_process = sp.Popen(
                    sender_cmd.split(), universal_newlines=True)
            else:
                sender_process = sp.Popen(
                    sender_cmd.split(), stdin=sp.PIPE, stdout=sp.PIPE, universal_newlines=True)
            ...
            if verbose >= 2:
                ...
                sender_process = sp.Popen(
                    sender_cmd.split(), stdin=sp.PIPE, stdout=sp.PIPE)
            ...
            raise Exception(
                'mm_side option should be \'sender\' or \'receiver\' only')
        ...
        plot_tput_delay(savepathprefix + '_downlink.csv',
                        title=os.path.basename(savepathprefix), disp=disp_plot, save=save_plot)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Run mahimahi for cellular network simulations")
    ...
    sim = Simulation(args.trace, args.port, args.ttr, args.n_blocks,
                     args.filepath, args.blksize, args.cc_algo, args.buf_len, args.iperf)

    sim.run(args.dir, args.mm_side, args.log,
            args.verbose, args.disp_plot, args.save_plot)
