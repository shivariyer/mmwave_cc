...
def RunSim():
    server_ip = "100.64.0.1" # MAHIMAHI_BASE
    ...
    recvlogfpath = os.path.join(args.dir, saveprefix + '_receiver.log')
    receiver_cmd = 'sender_receiver/receiver {} {} 1 &'.format(args.port, recvlogfpath)
    ...
    mm_process = subprocess.Popen(mm_cmd, stdin=subprocess.PIPE, universal_newlines=True, shell=True) # add text=True if required (may be required in more recent python3 versions such as python 3.7.x
    ...
    print(sender_cmd)
    stdout, stderr = mm_process.communicate(sender_cmd)
    ...
    # print(receiver_process.wait())
    # subprocess.call("kill {}".format(receiver_process.pid), shell=True)
    subprocess.call("pkill receiver", shell=True)
    print('Done!')
