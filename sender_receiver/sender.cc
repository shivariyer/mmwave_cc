...
int setup_tcp_connection(char *serv_ip, int serv_port, char *cc_protocol, struct tcp_conn *conn)
{
...
int send_ttr(struct tcp_conn *conn, unsigned int ttr, unsigned int blksize, bool probe)
{
...
int send_nblocks(struct tcp_conn *conn, int n_blocks, unsigned int blksize, bool probe)
{
...
int send_fromtrace(struct tcp_conn *conn, char *tracefilepath)
{
...
/**
 * Runs a shell command and returns its text output
 * Param: shell command
 * Returns: text output of shell commmand
 */
string system_get_output(const char *cmd)
{
...
string get_interface(string interface_name)
{
  ...
  {
    cerr << "regex_search() failed in sender, regex_str: " + regex_str << endl;
  }
...
int main(int argc, char **argv)
{
  ...
  cout << "lets print something 1" << endl;
  int status;
  pid_t pid = fork();
  ...
  else
  {
    ...
    while ((pid = waitpid(-1, &status, 0)) > 0)
      cout << "Server child " << pid << " with tshark has terminated." << endl;
  }

  //TODO kill the the specific pid instead
  //Close wireshark
  //system("pkill tshark");
